...
const ImageUploadManager: React.FC<ImageUploadManagerProps> = ({initialImages = [], onImagesChange}) => {
    ...
    const handleFileSelected = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            if (uploadResponse.data?.status === 1) {
                const {file: newFileName, path} = uploadResponse.data.data;
                const newImage: ProductImage = {
                    image: newFileName,
                    containerName: path,
                    defaultImage: images.length === 0 ? 1 : 0,
                };
    ...
    return (
        <View style={styles.wrapper}>
            ...
            <Pressable style={styles.addButton} onPress={handleAddImageClick} disabled={uploading}>
                <Text style={styles.addButtonText}>+ Add Image</Text>
            </Pressable>

            {/* This is the hidden file input for the web */}
            <input
                type="file"
                ref={inputRef}
