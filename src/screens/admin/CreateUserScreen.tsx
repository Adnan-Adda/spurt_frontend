...
// const CreateUserScreen = () => {
//     // --- DEBUG LOG ---
//     console.log('[CreateUserScreen] Component is attempting to render.');
//
//     const [roles, setRoles] = useState<Role[]>([]);
//     const [loading, setLoading] = useState(false);
//     const [error, setError] = useState<string | null>(null);
...
//         const fetchRoles = async () => {
//             // --- DEBUG LOG ---
//             console.log('[CreateUserScreen] useEffect triggered. Fetching roles...');
//             try {
//                 const response = await getRolesApi();
//                 if (response.data && response.data.status === 1) {
//                     console.log('[CreateUserScreen] Successfully fetched roles:', response.data.data.length);
//                     setRoles(response.data.data);
//                     if (response.data.data.length > 0) {
//                         setForm(prev => ({ ...prev, userGroupId: response.data.data[0].groupId }));
//                     }
//                 } else {
//                     throw new Error(response.data.message || 'API returned failure status for roles.');
//                 }
//             } catch (e: any) {
//                 console.error('[CreateUserScreen] Failed to load user roles:', e.message);
//                 setError('Failed to load user roles.');
//             }
//         };
//         fetchRoles();
...
//         try {
//             const payload = { ...form, username: form.email };
//             console.log('[CreateUserScreen] Attempting to create user with payload:', payload);
//             const response = await createUserApi(payload);
//             if (response.data && response.data.status === 1) {
//                 Alert.alert('Success', 'User created successfully!', [
//                     { text: 'OK', onPress: () => navigation.goBack() }
...
//             }
//         } catch (err: any) {
//             console.error('[CreateUserScreen] Error creating user:', err.message);
//             setError(err.message || 'An unknown error occurred.');
//         } finally {
//             setLoading(false);
//         }
...
//
//     // --- DEBUG LOG ---
//     console.log('[CreateUserScreen] Rendering JSX...');
//
//     return (
//         <SafeAreaView style={styles.safeArea}>
//             <ScrollView style={styles.container}>
